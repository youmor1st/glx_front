...
interface TopSwapsSelectDrawerProps {
  onFilterSubmit: ({
    swapsMin,
    swapsMax,
  }: {
...
export default function TopSwapsSelectDrawer({
  onFilterSubmit,
}: TopSwapsSelectDrawerProps) {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    console.log(values);
    onFilterSubmit({
      swapsMax: Number(values.higherRange),
      swapsMin: Number(values.lowerRange),
    });
    setOpen(false);
  };
                ...
                render={({ field }) => (
                  <FormItem className="gap-[12px]">
                    <FormControl>
                      ...
                      >
                        {[
                          ["Swaps: High to Low", "HtL"],
                          ["Swaps: Low to High", "LtH"],
                        ].map(([label, value]) => (
                          <RadioSelectCard
                            key={value}
                            label={label}
